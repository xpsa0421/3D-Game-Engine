...
{
	HRESULT hr;
	hr = _d2dRT->CreateSolidColorBrush(_textColor, &_colorBrush);
	return hr;
}
...
bool Writer::Frame()
{
	_textContent2 = L"FPS: " + std::to_wstring(g_FPS);
	return true;
}

bool Writer::Render()
{
	Draw(10, 5, _textContent, _textColor);
	Draw(g_rectClient.right - 100, 5, _textContent2, _textColor);
	return true;
}
...
bool Writer::Draw(float x, float y, W_STR text, D2D1_COLOR_F color)
{
	_d2dRT->BeginDraw();
